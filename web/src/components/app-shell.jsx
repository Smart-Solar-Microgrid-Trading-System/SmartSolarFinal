...
const sidebarBackground =
  "bg-[linear-gradient(180deg,var(--color-brand-950)_0%,#0e2847_55%,var(--color-brand-950)_100%)]";
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="relative grid size-10 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-brand-400 via-brand-500 to-solar-500 text-white shadow-lg shadow-brand-900/30">
        <Zap size={20} aria-hidden="true" className="drop-shadow" />
      </div>
      ...
      {!compact && (
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-white/50">Trading System</p>
        </div>
      )}
    </div>
  );
        ...
        >
          {({ isActive }) => (
            <>
              ...
              {isActive && (
                <ChevronRight size={15} className="ml-auto shrink-0 text-white/80" aria-hidden="true" />
              )}
            </>
          )}
        </NavLink>
...
function UserCard({ session, signOut, tone = "dark" }) {
  ...
  return (
    <div className={cn("flex items-center gap-3 rounded-xl p-3", isDark ? "bg-white/[0.06]" : "bg-slate-50")}>
...
export function AppShell() {
  ...
  // users and prosumers are only for backoffice, the rest use the roles list
  const navigationForRole = navigation.filter((item) => {
    if (["/users", "/prosumers"].includes(item.to)) {
      return session?.role === "Backoffice";
    }
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <aside
        className={cn(
          "sticky top-0 hidden h-screen w-64 shrink-0 flex-col overflow-hidden md:flex",
          sidebarBackground
        )}
      >
        <div
        ...
        <div className="sidebar-scroll relative z-10 flex-1 overflow-y-auto px-3 py-4">
          <p className="px-3 pb-2 text-[11px] font-semibold uppercase tracking-wider text-white/35">Menu</p>
          <Navigation items={navigationForRole} />
        </div>

        <div className="relative z-10 border-t border-white/10 p-3">
          <UserCard session={session} signOut={signOut} tone="dark" />
        </div>
      </aside>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <Sheet>
              ...
              <SheetContent
                side="left"
                className={cn(
                  "flex w-[280px] flex-col gap-0 border-none p-0 text-white [&_svg]:text-white",
                  sidebarBackground
                )}
              >
            ...
            <div className="hidden min-w-0 md:block">
              ...
              <h1 className="truncate font-display text-base font-semibold text-slate-900">{currentSection}</h1>
            </div>
          </div>
        ...
        <main className="min-w-0 flex-1 p-4 sm:p-6 lg:p-8">
          {/* key on the path so the page animation runs on every route change */}
          <div key={location.pathname} className="animate-page-in mx-auto w-full max-w-7xl">
            <Outlet />
          </div>
        </main>
      </div>
